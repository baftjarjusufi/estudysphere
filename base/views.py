...
#rooms = [
#    {'id':1, 'name':'Lets Learn Finals'},
#    {'id':2, 'name':'Lets make projects'},
#    {'id':3, 'name':'Lets finish assigments'},
#
#]
...
def home(request):
    ...
    roommessages = Message.objects.filter(Q(room__topic__name__icontains=q))[0:4]
...
def room(request, pk):
    ...
    roommessages = room.message_set.all() #we query all the parents models from models.py basically all corelated to the room model
    entrants = room.entrants.all()
    ...
    if request.method == 'POST':
        ...
        room.entrants.add(request.user)
        return redirect('room', pk=room.id)
    
    
    context ={'room': room, 'roommessages': roommessages, 'entrants': entrants}
...
def updateRoom(request, pk):
    ...
    if request.user != room.host:
        return HttpResponse('Permission denied , you are not accessed!!')
